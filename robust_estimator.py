...
def mom_ex_noregret(samples, eps=0.2, sigma=1, expansion=20, itv=ITV, delta=np.exp(-30)):
    ...
    for i in range(bucket_num):
        bucket = samples[i * bucket_size:min((i + 1) * bucket_size, len(samples))]
        if len(bucket) !=0:
            bucketed_samples.append(np.mean(bucket, axis=0))
        # print(np.mean(samples[i*bucket_size:min((i+1)*bucket_size, len(samples))], axis=0))
